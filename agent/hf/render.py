...
def _add_helper_functions(env: sandbox.ImmutableSandboxedEnvironment):
    """
    Register any helper functions required by your template.
    Currently we only need `strftime_now`.
    """

    def strftime_now(fmt: str = "%Y-%m-%dT%H:%M:%S"):
        """
        Return UTC time in ISO 8601.
        This matches the behaviour of llama.cpp's OpenAI Harmony format.
        """
...
def render_chat_template(
    ...
    if render_template:
        # Render the formatted template using Jinja2 with a context that
        # includes 'bos_token' and 'eos_token'
        env = sandbox.ImmutableSandboxedEnvironment(trim_blocks=True, lstrip_blocks=True)
        _add_helper_functions(env)
        ...
        messages = [
            ...
            {
                "role": "assistant",
                "content": "Certainly! What would you like to know about it?",
            },
            {
                "role": "user",
                "content": "Is it just regular mayonnaise with vinegar or something else?",
            },
        ]
