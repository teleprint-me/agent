...
# this is a drafted call generated by gpt-oss
_generated = r"""
# On each revision:
git checkout <rev>
python - <<'PY'
...
def run(cmd):
    res = subprocess.run(["agent_shell", cmd], capture_output=True, text=True)
    if res.returncode != 0 or "early" in res.stdout.lower():
        print("FAIL on:", cmd)
        return False
    return True

# Step‑1: list directory (tool call #1)
if not run('ls -al .'): sys.exit(1)

# Step‑2: read a file (tool call #2)
run('head -n 10 README.md')
PY
"""
